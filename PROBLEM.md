...
| 8   | [[STRC ASO Exon Skipping]]                            | 3→**2** | 3     | 2     | Phase 1 design (54 candidates); local self-fold **PASS 54/54** ([[STRC ASO Phase1 Fold Check]]); **Phase 2 STRCP1 cross-hybridization FAIL 0/54** ([[STRC ASO Phase2 STRCP1 Paralog Cross-Hybridization]]) — every Phase-1 ASO hits STRCP1 with ≤2 mm (44 perfect, 10 at 1 mm). Splice-site conservation between STRC and STRCP1 (≥97 % identity) is the kill mechanism. Mirrors PE Phase 4 STRCP1 finding. | backburner                              | **B → C**   | (a) verify STRCP1 OHC expression via human OHC RNA-seq (Zheng-Quayle 2022 / Kolla 2020) — shared gate with [[Prime Editing for STRC]]; if STRCP1 silent, paralog risk drops; (b) evaluate morpholino chemistry (no RNase-H cleavage, blocks splicing — STRCP1 makes no protein anyway); (c) evaluate allele-specific gapmer at c.4976 variant directly |
